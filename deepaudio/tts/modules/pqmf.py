...
class PQMF(nn.Module):
    ...
    def __init__(self, subbands=4, taps=62, cutoff_ratio=0.142, beta=9.0):
        ...
        # convert to tensor
        self.analysis_filter = torch.from_numpy(
            h_analysis.astype(np.float32)).unsqueeze(1)
        self.synthesis_filter = torch.tensor(
            h_synthesis.astype(np.float32)).unsqueeze(0)

        # filter for downsampling & upsampling
        updown_filter = torch.zeros(
            (subbands, subbands, subbands), dtype=torch.float32)
        for k in range(subbands):
            updown_filter[k, k, 0] = 1.0
        self.updown_filter = updown_filter
        self.subbands = subbands
        # keep padding info
        self.pad_fn = nn.ConstantPad1d(taps // 2, value=0.0)
