...
class Conv1dCell(nn.Conv1d):
    ...
    Examples: 
        ...
        >>> inputs = [torch.randn([4, 3]) for _ in range(16)]
        >>> outputs = []
        >>> cell.eval()
        >>> cell.start_sequence()
        >>> for xt in inputs:
    ...
    def start_sequence(self):
        ...
        for hook in self._forward_pre_hooks.values():
            hook(self, None)
        self._reshaped_weight = torch.reshape(self.weight,
    ...
    def update_buffer(self, x_t):
        ...
        """
        self._buffer = torch.concat(
            [self._buffer[:, :, 1:], torch.unsqueeze(x_t, -1)], -1)
    ...
    def add_input(self, x_t):
        ...
        if self.receptive_field > 1:
            ...
            input = x_t
        y_t = torch.matmul(input, self._reshaped_weight.transpose())
        y_t = y_t + self.bias
        return y_t
