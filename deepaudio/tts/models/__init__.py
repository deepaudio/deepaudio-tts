...
def register_model(name: str, dataclass=None):
    ...
    def register_model_cls(cls):
        ...
        if not issubclass(cls, BasePLModel):
    ...
    if os.path.isdir(os.path.join(models_dir, file)) and not file.startswith('__'):
        for subfile in os.listdir(os.path.join(models_dir, file)):
            ...
            if subfile.endswith(".py"):
                ...
                module = importlib.import_module(f"deepaudio.tts.models.{file}.{python_file}")
        continue

    path = os.path.join(models_dir, file)
    if file.endswith(".py"):
        model_name = file[: file.find(".py")] if file.endswith(".py") else file
        module = importlib.import_module(f"deepaudio.tts.models.{model_name}")