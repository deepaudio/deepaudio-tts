...
from deepaudio.tts.audio.codec import encode_mu_law
from deepaudio.tts.audio.codec import float_2_label
from deepaudio.tts.audio.codec import label_2_float
...
class WaveRNNClip(Clip):
    ...
    def __call__(self, batch):
        ...
        mels = default_collate(mels)
        labels = default_collate(labels.astype(np.int64))
        ...
        x = label_2_float(x.type(dtype=torch.float32), self.bits)
        # y should be normalizes in.[0, 1] in MOL mode
        if self.mode == 'MOL':
            y = label_2_float(y.type(dtype=torch.float32), self.bits)
