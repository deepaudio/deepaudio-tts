...
from deepaudio.tts.datasets.am_batch_fn import tacotron2_multi_spk_batch_fn
...
class GanDataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if self.hparams.voice_cloning:
            print("Training voice cloning!")
            self.collate_fn = tacotron2_multi_spk_batch_fn
            fields += ["spk_emb"]
            converters["spk_emb"] = np.load
        else:
            print("single speaker tacotron2!")
            self.collate_fn = tacotron2_single_spk_batch_fn
